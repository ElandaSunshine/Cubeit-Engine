...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    public void registerCubeitItems(final ObjectRegistry registry)
    {
    	...
    	for (final RegistryEntry<GameBlock> entry : registryBlocks)
    	{
			...
			final Item      itemBlock = Item.getItemFromBlock(null);
			...
			if (itemBlock != null)
			{
				...
				if (variants == null)
				{
					registerBlock(block, GameInfo.get().gameId + ":" + entry.getKey());
				}
				else
				{
					for (final int variant : variants)
					{
						registerBlock(block, variant, GameInfo.get().gameId + ":" + entry.getKey());
					}
				}
			}
    	}
    	...
    	for (final RegistryEntry<GameItem> entry : registryItems)
    	{
			...
			if (variants == null)
			{
				registerItem(item, GameInfo.get().gameId + ":" + entry.getKey());
			}
			else
			{
				for (final int variant : variants)
				{
					registerItem(item, variant, GameInfo.get().gameId + ":" + entry.getKey());
				}
			}
    	}
    }
