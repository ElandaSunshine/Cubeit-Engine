...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    public void registerCubeitItems(final ObjectRegistry registry)
    {
    	final Registry<GameBlock> registryBlocks = registry.blocks;
        final String id = GameManager.getInstance().gameInfo.gameId;

        for (final RegistryEntry<GameBlock> entry : registryBlocks)
    	{
			final GameBlock block     = entry.getValue();
			final Item      itemBlock = (block.getItemBlock() == null ? null : Item.getItemFromBlock(block));
			
			if (itemBlock != null)
			{
				final int[]  variants = block.getModelVariants();
				if (variants == null)
				{
					registerBlock(block, id + ":" + entry.getKey());
				}
				else
				{
					for (final int variant : variants)
					{
						registerBlock(block, variant, id + ":" + entry.getKey());
					}
				}
			}
    	}
    	
    	final Registry<GameItem> registryItems = registry.items;
    	
    	for (final RegistryEntry<GameItem> entry : registryItems)
    	{
    		final GameItem item     = entry.getValue();
			final int[]    variants = item.getModelVariants();
			
			if (variants == null)
			{
				registerItem(item, id + ":" + entry.getKey());
			}
			else
			{
				for (final int variant : variants)
				{
					registerItem(item, variant, id + ":" + entry.getKey());
				}
			}
    	}
    }
