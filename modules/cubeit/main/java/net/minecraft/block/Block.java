...
public class Block
{
    ...
    {    	
    	final Block.EnumOffsetType block$enumoffsettype = this.getOffsetType();
    	
        if (block$enumoffsettype == Block.EnumOffsetType.NONE || block$enumoffsettype == Block.EnumOffsetType.NONE)
        {
            return Vec3d.ZERO;
        }
        else
        {
            ...
            final double x = ((double)((float)(i >> 16 & 15L) / 15.0F) - 0.5D) * 0.5D;
            final double y = ((double)((float)(i >> 20 & 15L) / 15.0F) - 1.0D) * 0.2D;
            final double z = ((double)((float)(i >> 24 & 15L) / 15.0F) - 0.5D) * 0.5D;
            
            return new Vec3d(x, (block$enumoffsettype == Block.EnumOffsetType.XYZ ? y : 0.0D), z);
        }
    }
    ...
    {        
        for (int i = 0; i < registry.size(); ++i)
        {
        	...
        	registerBlock(ObjectRegistry.cubeitBlockStartId + i,
        			      GameManager.getInstance().gameInfo.gameId + ":" + entry.getKey(), entry.getValue());
        }
